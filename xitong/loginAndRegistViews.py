...
def userxiugaigerenxinxiact(request):
    ...
    user = models.User.objects.get(id=id);#  从POST中获取用户id信息，并赋值给user的id字段
    user.id = request.POST.get("id");#  从POST中获取用户id信息，并赋值给session的id字段
    request.session['id'] = request.POST.get("id");#  从POST中获取名字信息，并赋值给user的name字段
    user.name = request.POST.get("name");#  从POST中获取名字信息，并赋值给session的name字段
    request.session['name'] = request.POST.get("name");#  从POST中获取真实姓名信息，并赋值给user的realname字段
    user.realname = request.POST.get("realname");#  从POST中获取真实姓名信息，并赋值给session的realname字段
    request.session['realname'] = request.POST.get("realname");#  从POST中获取账号信息，并赋值给user的username字段
    user.username = request.POST.get("username");#  从POST中获取账号信息，并赋值给session的username字段
    request.session['username'] = request.POST.get("username");#  从POST中获取密码信息，并赋值给user的password字段
    user.password = request.POST.get("password");#  从POST中获取密码信息，并赋值给session的password字段
    request.session['password'] = request.POST.get("password");#  从POST中获取性别信息，并赋值给user的sex字段
    user.sex = request.POST.get("sex");#  从POST中获取性别信息，并赋值给session的sex字段
    request.session['sex'] = request.POST.get("sex");#  从POST中获取性别信息，并赋值给user的age字段
    user.age = request.POST.get("age");#  从POST中获取性别信息，并赋值给session的age字段
    request.session['age'] = request.POST.get("age");#  从POST中获取介绍信息，并赋值给user的introduce字段
    user.introduce = request.POST.get("introduce");#  从POST中获取介绍信息，并赋值给session的introduce字段
    request.session['introduce'] = request.POST.get("introduce");#  使用save方法保存用户信息
    user.save();#  给出页面提示修改成功，并跳转到用户个人中心
